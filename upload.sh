...
#修改spec文件tag
#获取版本号并显示
VersionString=`grep -E "s.version.*=" GHKmpShare.podspec`
...
LineNumber=`grep -nE 's.version.*=' GHKmpShare.podspec | cut -d : -f1`
sed -i "" "${LineNumber}s/${VersionNumber}/${NewVersionNumber}/g" GHKmpShare.podspec
echo -e "\n${Default}================================================"
